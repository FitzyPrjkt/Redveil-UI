...
from redveil_ui.api.schemas import (
    SiteMapEndpointOut,
    SiteMapOut,
    TargetCreate,
    TargetOut,
...
async def create_target(
    ...
    # Synchronous scope check: refuse to persist a target whose own URL
    # is rejected by its scope_yaml. Defense-in-depth — the orchestrator
    # would refuse the same request mid-run, but persisting it creates
    # a row that the UI surfaces as a launchable target.
    ok, reason = check_target_url_in_scope(body.url, body.scope_yaml)
    if not ok:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"target url is out of scope: {reason}",
        )
...
async def update_target(
    ...
    if body.scope_yaml is not None:
        ...
        # When scope changes, re-validate the target URL against the
        # new scope. Without this, an operator could widen hosts in a
        # way that contradicts the existing URL.
        ok, reason = check_target_url_in_scope(target.url, target.scope_yaml)
        if not ok:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=f"new scope rejects this target's url: {reason}",
            )
    await session.commit()
    await session.refresh(target)
    return target
