...
def _build_config(
    ...
    if scope_yaml:
        try:
            ...
            # Fail closed: a malformed scope_yaml must NOT silently become
            # "allow everything" (the auto-allow default above). The API
            # layer (scope_check.py + Pydantic validators) already rejects
            # malformed YAML at the request boundary; reaching this point
            # with a malformed YAML means either (a) an older persisted
            # target row whose scope_yaml was accepted by an earlier
            # build, or (b) internal code that bypassed the API layer.
            # Either way, fail closed and surface the parse error.
            raise ValueError(
                f"scope_yaml is not valid YAML: {e}. Refusing to run with "
                f"the auto-allow default scope (which would permit any host). "
                f"Fix the scope_yaml in the target row, or remove it to fall "
                f"back to the auto-allow default for a well-formed config."
            ) from e
...
class Scanner:
    ...
    def output_base_dir(self) -> Path:
        """Public accessor for the configured output directory."""
