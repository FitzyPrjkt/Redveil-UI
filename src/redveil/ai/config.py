...
class AiProviderConfig(BaseModel):
    ...
    base_url: str = Field(..., description="Gateway base URL, e.g. https://any-proxy.web.id/v1 or http://localhost:11434/v1")
    api_key: str | None = Field(default=None, description="API key (prefer api_key_env)")
    api_key_env: str | None = Field(default=None, description="Env var name for API key, e.g. ANY_PROXY_API_KEY")
    model: str = Field(..., description="Model identifier, e.g. gpt-4o-mini or custom-model-x")
    ...
    def resolved_api_key(self) -> str | None:
        """Resolve API key from api_key_env or direct api_key (redacted in logs)."""
        if self.api_key_env:
